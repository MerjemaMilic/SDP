...
public class KosevoActivity extends AppCompatActivity {
    ...
    DatabaseReference referenceRatingkosevo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kosevo);
        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.prijedorrecycler);
        ...
        referenceRatingkosevo = FirebaseDatabase.getInstance().getReference("ratings/Koševo");

        referenceRatingkosevo.addValueEventListener(new ValueEventListener() {
            @Override
        ...
        Query query = firebaseFirestore.collection("reviews").whereEqualTo("neighborhood","Koševo");

        FirestoreRecyclerOptions<Rate> options = new FirestoreRecyclerOptions.Builder<Rate>()
                .setQuery(query, Rate.class)
                .build();

        adapter = new FirestoreRecyclerAdapter<Rate, KosevoActivity.ProductsViewHolder>(options) {
            @NonNull
            @Override
            public KosevoActivity.ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.custom_view, parent, false);
                return new KosevoActivity.ProductsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull KosevoActivity.ProductsViewHolder productsViewHolder, int i, @NonNull Rate rate) {
